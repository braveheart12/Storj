...
func TestDisqualifiedNodesGetNoDownload(t *testing.T) {
	...
	testplanet.Run(t, testplanet.Config{
		...
		encParameters := upl.GetConfig(sat).GetEncryptionParameters()
		...
		pointer, err := sat.Metainfo.Service.Get(ctx, lastSegPath)
		require.NoError(t, err)
		...
		for _, orderLimit := range limits {
			assert.False(t, isDisqualified(t, ctx, sat, orderLimit.Limit.StorageNodeId))
...
func TestDisqualifiedNodesGetNoUpload(t *testing.T) {
	...
	testplanet.Run(t, testplanet.Config{
		...
		disqualifyNode(t, ctx, sat, disqualifiedNode.ID())
		...
		nodes, err := sat.Overlay.Service.FindStorageNodes(ctx, request)
		assert.True(t, overlay.ErrNotEnoughNodes.Has(err))

		assert.Len(t, nodes, 3)
		for _, node := range nodes {
			assert.False(t, isDisqualified(t, ctx, sat, node.Id))
...
func TestDisqualifiedNodeRemainsDisqualified(t *testing.T) {
	...
	testplanet.Run(t, testplanet.Config{
		...
		_, err = sat.DB.OverlayCache().UpdateUptime(ctx, disqualifiedNode.ID(), true, 0, 1, 0)
		require.NoError(t, err)

		assert.True(t, isDisqualified(t, ctx, sat, disqualifiedNode.ID()))
